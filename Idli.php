...
body 
{
  ...
  background-color:#B9D9EB;
    ...
    .idli
    {
        ...
        border-radius:20px;
        background: linear-gradient(#e66465, #91c3e5);
        top:200px;
        left:300px;
    }
    .Home
...
.part2
{
  ...
  top:50px;
  left:500px;
  background: rgb(231,241,238);
  background: radial-gradient(circle, rgba(231,241,238,1) 0%, rgba(35,222,198,1) 100%);
  background-color:white;
  border: 2px solid black;
  border-radius:20px;
  height:400px;
...
<body>
<?php
include "ConnectionsAss.php";
$validate=TRUE;
$item='IDLI';
$FoodName=$Price=$Quantity=$Offers='';
if($_SERVER["REQUEST_METHOD"]=="POST")
    {
        $sql="SELECT * FROM Food WHERE FoodCode='$item'";
        $res=mysqli_query($conn,$sql);
        $ass=mysqli_fetch_assoc($res);
        if($ass)
        {
            if($ass['FoodCode']==$item )
            {
                    $FoodName=$ass['FoodName'];
                    $Price=$ass['Price'];
...
?>
  ...
  <div class="idli">
    <div class="part1"></div>
    <div class="part2">
    <h2 style="margin-left:155px; text-decoration:underline;background:transparent;">IDLI</h2><br><br>
    <h5 style="margin-left:35px;background:transparent;">Price: 200/-</h5><br>
    <h5 style="margin-left:35px;background:transparent;">Quantity: 1</h5><br>
    <h5 style="margin-left:35px;background:transparent;">Offers: 20% off for students</h5><br><br>
    <input type="button" onClick="location.href='idli_pay.html'" style=" height:35px; width:130px;border-radius:7px;background: rgb(106,230,94);
    background: radial-gradient(circle, rgba(106,230,94,1) 0%, rgba(162,212,134,1) 100%);font-size:16px;margin-left:60px;" value="OrderNow">
    </div>
  </div>
</body>
</html>